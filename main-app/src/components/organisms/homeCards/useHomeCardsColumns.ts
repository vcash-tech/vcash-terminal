...
export const useHomeCardsColumns = ({ isFullScreen: _ }: UseHomeCardsColumnsProps) => {
    ...
    const createRandomCard = useCallback((bottomPosition: number) => {
        const sportCards = homeCardsData.filter(card =>
            card.type === ProductCardType.socker || card.type === ProductCardType.basketball || card.type === ProductCardType.f1)
        const casinoCards = homeCardsData.filter(card =>
            card.type === ProductCardType.casino || card.type === ProductCardType.casino2)
        const gamingCards = homeCardsData.filter(card =>
            card.type === ProductCardType.steam || card.type === ProductCardType.playstation || card.type === ProductCardType.xbox)

        const random = Math.random()
        let selectedCard

        if (random < 0.5 && sportCards.length > 0) {
            selectedCard = sportCards[Math.floor(Math.random() * sportCards.length)]
        } else if (random < 0.8 && casinoCards.length > 0) {
            selectedCard = casinoCards[Math.floor(Math.random() * casinoCards.length)]
        } else {
            selectedCard = gamingCards[Math.floor(Math.random() * gamingCards.length)]
        }

        if (!selectedCard) {
            selectedCard = homeCardsData[Math.floor(Math.random() * homeCardsData.length)]
        }
    ...
    const initializeColumns = useCallback(() => {
        ...
        for (let i = 0; i < numColumns; i++) {
            ...
            const cardsNeeded = Math.ceil(containerHeight / CARD_WITH_GAP) * 3 + 5
        ...
        setColumns(prevColumns => {
            return prevColumns.map(column => {
                ...
                const updatedCards = column.cards.map(card => {
                    ...
                    return {
                        ...card,
                        bottomPosition: card.bottomPosition - (column.animationSpeed * deltaTime)
                    }
                })
                ...
                updatedCards.forEach(card => {
                    if (card.bottomPosition < -window.innerHeight - CARD_HEIGHT && !card.removed) {
                        card.removed = true
                ...
                const visibleCards = filteredCards.filter(card => !card.removed)
                ...
                if (bottomCard && bottomCard.bottomPosition < CARD_WITH_GAP * 3) {
                    const cardsToAdd = Math.max(1, Math.ceil(CARD_WITH_GAP * 3 / CARD_HEIGHT))
                    for (let i = 0; i < cardsToAdd; i++) {
                        // Add new cards at increasing bottom positions
                        // This ensures they appear below the current bottom card
                        const newBottomPosition = bottomCard.bottomPosition + CARD_WITH_GAP * (i + 1)
                        filteredCards.push(createRandomCard(newBottomPosition))
                    }
                }
                ...
                if (columns.length < newNumColumns) {
                    ...
                    setColumns(prevColumns => {
                        ...
                        for (let i = columns.length; i < newNumColumns; i++) {
                            ...
                            const cardsNeeded = Math.ceil(containerHeight / CARD_WITH_GAP) * 3 + 5
                            for (let j = 0; j < cardsNeeded; j++) {
                                const yPosition = -CARD_HEIGHT + (j * CARD_WITH_GAP)
                    ...
                    setColumns(prevColumns => prevColumns.slice(0, newNumColumns))
                }
            }
        }
    ...
    }, [columns.length, createRandomCard, getRandomAnimationSpeed, CARD_WITH_GAP])
    ...
    const getVisibleCards = useCallback((columnId: number) => {
